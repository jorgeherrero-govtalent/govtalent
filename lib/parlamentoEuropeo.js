...
//     vivo: fase, cierre, actos y ponentes, y añade a la cronología los
//     actos nuevos. Es lo que vigila el motor de avisos.
//
//   sincronizarEventos()  → /api/sync/parlamento-procedimientos/eventos
//     Trabajo de fondo: completa por turnos la cronología de los vivos
//     y va vaciando el atraso de cerrados. Ver "DE DÓNDE SALE LA
...
//
// DE DÓNDE SALE LA CRONOLOGÍA, desde el 20 de septiembre de 2026.
// Pedir la cronología de cada vivo cada noche no cabe: cada una tarda
// 11-20 s en llegar y ni con 6 a la vez se pasó de 0,30 por segundo
// (230 vivos en 783 s, a 17 s del límite de la ruta). Pero la FICHA, que
// ya se pide cada noche y llega rápido, trae la lista de actos
// (consists_of): comprobado ese día, los 1.646 actos de las fichas de
// los vivos estaban todos en la cronología, con el mismo id y los mismos
// campos. Solo 47 eventos de la cronología no estaban en la ficha, casi
// todos PLENARY_ACTIVITY de 2019-2023. Así que:
//
//   - Las FICHAS añaden cada noche los actos nuevos de cada procedimiento.
//     Solo inserta los que no existen: nunca pisa lo que trajo la
//     cronología completa.
//   - Las CRONOLOGÍAS pasan a ser trabajo de fondo: repasan por turnos los
//     vivos —cada uno, al menos cada dos días— y vacían el atraso de
//     cerrados, con lo que quepa. Solo cuenta como "dejó trabajo" si algún
//     vivo pasa de 75 horas sin cronología completa.
//
...
// servidor pasaba tiempo parado. Ahora siempre hay 3 peticiones en
...
// 30 s de espera. Una cronología tarda unos 11 s con 3 en vuelo. Antes
...
// Con 3 en vuelo no hubo ni un corte por tiempo de espera en toda una
// noche. Con 6, las cronologías pasaron de tardar 11 s a 20 s cada una:
// la API no da más, solo reparte más despacio.
const EN_VUELO = 3;

// Las cronologías tienen menos tiempo: ya no hace falta apurar y así la
// ejecución cierra con 2-3 minutos de margen sobre los 800 s.
const PRESUPUESTO_EVENTOS_MS = 600_000;

// Rotación de cronologías de los vivos. Cada noche van primero los que
// llevan más de ROTACION_H horas sin cronología completa: así cada vivo
// pasa como mucho cada dos días. Es 36 y no 48 a propósito: uno repasado
// hace dos noches tiene 47 h y pico, no "más de 48", y esperaría una
// tercera. Si al terminar algún vivo pasa de ALERTA_H —tres noches y
// algo de holgura—, la ejecución sale como "dejó trabajo" y se ve en la
// pantalla de syncs: significa que la API no da para el ritmo previsto.
const ROTACION_H = 36;
const ALERTA_H = 75;
...
async function escribir(supabase, tabla, todas, conflicto, { soloNuevos = false } = {}) {
  ...
  for (let i = 0; i < filas.length; i += LOTE) {
    ...
    // soloNuevos: ON CONFLICT DO NOTHING. Devuelve solo las insertadas.
    const { data, error } = await supabase
      .from(tabla)
      .upsert(grupo, { onConflict: conflicto, ignoreDuplicates: soloNuevos })
      .select(conflicto);
...
// Una actividad del PE como fila de ep_procedure_events. La usan las dos
// fuentes: la cronología y la lista de actos de la ficha, que traen los
// mismos campos.
function filaEvento(ev, pid) {
 ...
 */
async function enPiscina(items, pedirUno, { t0, tratar, volcar, enVuelo = EN_VUELO, presupuestoMs = PRESUPUESTO_MS }) {
...
export async function sincronizarFichas({ supabase, dry = false, desde = 2014, hasta, t0 = Date.now() }) {
  ...
  const escritura = { procedimientos: { escritas: 0 }, participaciones: { escritas: 0 }, actos_nuevos: { escritas: 0 } };
  ...
  async function volcarFichas() {
    ...
    // Los actos nuevos de cada ficha, a la cronología. Solo los que no
    // existen: los que ya están los trajo la cronología completa y no se
    // tocan. Y solo de los procedimientos que se guardaron.
    const actosGuardados = actos.filter((a) => wP.claves.has(a.process_id));
    const wA = await escribir(supabase, 'ep_procedure_events', actosGuardados, 'id', { soloNuevos: true });
    escritura.actos_nuevos.escritas += wA.escritas;
    erroresEscritura.push(...wA.errores);
  ...
  if (!parado) {
    const r2 = await enPiscina(aPedir, (pid) => pedir(`${EP}/procedures/${pid}?${FORMATO}`), {
      ...
      tratar: (pid, r) => {
        ...
        bufProc.push(fila);
        bufPart.push(...parts);
        for (const a of Array.isArray(p.consists_of) ? p.consists_of : []) {
          const ev = filaEvento(a, fila.process_id);
          if (ev) bufActos.push(ev);
        }
...
/**
 * Trabajo de fondo: la cronología completa, por turnos.
 *
 * Los actos nuevos de los vivos ya los añade cada noche el sync de fichas.
 * Esto completa lo que la ficha no trae —sobre todo PLENARY_ACTIVITY— y
 * vacía el atraso de cerrados. ORDEN, con lo que quepa en el tiempo:
 *
 *   1. Vivos que llevan más de ROTACION_H horas sin cronología completa,
 *      el más antiguo primero. Así cada vivo pasa al menos cada dos días.
 *   2. Cerrados pendientes, los cerrados más recientemente primero: les
 *      falta la cronología final y es lo que alguien puede estar mirando.
 *      El 20 de septiembre había unos 890, de antes de existir esta fase.
 *   3. El resto de vivos, el más antiguo primero.
 *
 * Solo cuenta como "dejó trabajo" si al terminar algún vivo lleva más de
 * ALERTA_H horas sin cronología completa. Quedarse sin tiempo con el
 * atraso de cerrados o con vivos recientes es lo normal y sale como ok.
 */
export async function sincronizarEventos({ supabase, dry = false, t0 = Date.now() }) {
  ...
  // El orden se hace aquí y no en la consulta: son tres grupos con
  // criterios distintos, que una sola ORDER BY no expresa.
  const txt = (v) => (v ? String(v) : '');
  const ms = (v) => (v ? Date.parse(v) : NaN);
  const hace = (h) => t0 - h * 3600_000;
  // Sin fecha, o con una que no se entiende, cuenta como "nunca".
  const anteriorA = (x, h) => !(ms(x.events_synced_at) >= hace(h));
  const porAntiguedad = (x, y) => txt(x.events_synced_at).localeCompare(txt(y.events_synced_at)) || txt(x.process_id).localeCompare(txt(y.process_id));

  const vivosF = (filas || []).filter((x) => !x.is_closed);
  const cerradosF = (filas || []).filter((x) => x.is_closed);
  const vivosViejos = vivosF.filter((x) => anteriorA(x, ROTACION_H)).sort(porAntiguedad);
  const vivosRecientes = vivosF.filter((x) => !anteriorA(x, ROTACION_H)).sort(porAntiguedad);
  ...
  const pendientes = [...vivosViejos, ...cerradosF, ...vivosRecientes].map((x) => x.process_id);

  informe.vivos = vivosF.length;
  informe.vivos_a_repasar = vivosViejos.length;
  ...
  async function volcarEventos() {
    ...
    bufHechos = [];
    if (dry) {
      for (const pid of listos) marcados.add(pid);
      return true;
    }
    ...
    else for (const pid of listos) marcados.add(pid);
    return true;
  }
  ...
  const r = await enPiscina(pendientes, (pid) => pedir(`${EP}/procedures/${pid}/events?${FORMATO}`), {
    ...
    presupuestoMs: PRESUPUESTO_EVENTOS_MS,
  });
  let parado = r.parado;
  ...
  // Lo que de verdad importa: ¿queda algún vivo con la cronología
  // completa demasiado vieja? Da igual el motivo —tiempo, fallos de la
  // API, escritura rechazada—: si pasa de ALERTA_H, se tiene que ver.
  const rezagados = vivosF.filter((x) => !marcados.has(x.process_id) && anteriorA(x, ALERTA_H)).map((x) => x.process_id);
  if (rezagados.length) informe.vivos_rezagados = rezagados.slice(0, 10);
  const restanCerrados = cerradosF.length - hechosCerrados;
  if (restanCerrados > 0) informe.atraso_cerrados = restanCerrados;
  const vivosSinRepasar = vivosF.length - hechosVivos;

  const grave = parado === 'limite' || parado === 'escritura';
  return veredicto(informe, {
    parado: grave ? parado : rezagados.length ? 'rezagados' : null,
    erroresEscritura,
    fallidos: [],
    t0,
    ...
    cortadoPorTiempo: parado === 'tiempo' && (restanCerrados > 0 || vivosSinRepasar > 0),
    resumen: `${hechosVivos} de ${vivosF.length} vivos, ${vivosViejos.length} tocaban; ${hechosCerrados} de ${cerradosF.length} cerrados pendientes`,
  });
}
...
function veredicto(informe, { parado, erroresEscritura, fallidos, t0, hechos, total, que, sinFicha, cortadoPorTiempo = false, resumen = null, rezagados = 0 }) {
  ...
  if (parado === 'limite') {
  ...
  } else if (parado === 'rezagados') {
    informe.quedan = true;
    partes.push(`${rezagados} vivo(s) llevan más de ${ALERTA_H} h sin cronología completa. Van primero mañana.`);
    ...
    // Solo llega aquí la fase de cronologías: ningún vivo va con retraso y
    // lo que no dio tiempo sigue por turnos mañana.
    partes.push(`Vivos al día. Se acabó el tiempo previsto: ${informe.atraso_cerrados ?? 0} cerrados de atraso siguen mañana.`);
