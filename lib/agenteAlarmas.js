...
// Tiempo máximo por llamada. Sin él, una llamada colgada se comía todo
// el tiempo de la función y el usuario no recibía ni un error.
...
async function llamarIA(system, user, maxTokens = 2000) {
  ...
  try {
    ...
    throw new Error(e.name === 'AbortError' ? 'La IA ha tardado demasiado en responder' : `No se pudo llamar a la IA: ${e.message}`);
  } finally {
    clearTimeout(reloj);
  }
  if (!res.ok) {
  ...
  if (data.stop_reason === 'max_tokens') {
    throw new Error(`La respuesta de la IA llegó cortada (tope de ${maxTokens} tokens)`);
  }
  return leerJSON(texto);
}

/**
 * El JSON de la respuesta, aunque venga envuelto en ``` o con una frase
 * antes o después: se toma desde el primer { o [ hasta el último } o ].
 */
function leerJSON(texto) {
  ...
  try {
    ...
    throw new Error(`No se pudo leer la respuesta de la IA: ${e.message}`);
  }
}
...
export async function proponer(texto, web = '', dominio = '') {
  ...
  // 2.000 y no 900: con una descripción rica (varias líneas de normativa,
  // territorios y temas) la respuesta pasaba de 900 tokens, llegaba
  // cortada, el JSON no se podía leer y la alarma fallaba entera.
  const c = await llamarIA(SISTEMA_CRITERIOS, entrada, 2000);
  return {
...
export async function candidatos(supabase, keywords, { limite = MAX_CANDIDATOS, alBuscar } = {}) {
  ...
  let hechas = 0;
  // En paralelo y no una detrás de otra: quince consultas seguidas eran
  // hasta quince segundos de espera. Cada una avisa al terminar, así que
  // en pantalla los términos se siguen encendiendo uno a uno.
  const resultados = await Promise.all(
    validas.map(async (k) => {
      ...
      if (alBuscar) alBuscar({ termino: k, hecho: hechas, total: validas.length, encontrados: vistos.size });
      return data || [];
    })
  );
  void resultados;
