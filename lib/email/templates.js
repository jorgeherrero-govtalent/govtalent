...
export function welcomeCandidateEmail({ firstName }) {
  ...
  const html = shell(
    ...
    ${lista([
      'Acceder a un directorio institucional de España y la Unión Europea, con más de 11.000 cargos y contactos.',
...
export function welcomeOrganizationEmail({ orgName, firstName }) {
  ...
  const html = shell(
    ...
    ${lista([
      ...
      'Acceder al directorio institucional de España y la Unión Europea, con más de 11.000 cargos y contactos.',
 ...
 */
export function weeklyDigestEmail({ firstName, novedades = [], plazos = [], publicado = [], totalSeguidos = 0, sinTemas = false, unsubscribeUrl }) {
  ...
  const bloquePublicado =
    b === 0
      ...
      <div style="font-size:14.5px;line-height:1.55;margin-bottom:14px;letter-spacing:-.1px;">
        ...
        <a href="${SITE_URL}${x.ruta || '/boe'}" style="color:#1a1a18;text-decoration:none;">${x.title}</a><br/>
