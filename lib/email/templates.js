 ...
 */
export function weeklyDigestEmail({ firstName, novedades = [], plazos = [], publicado = [], totalSeguidos = 0, unsubscribeUrl }) {
...
export function dailyAlertEmail({ firstName, matches = [], total = 0, unsubscribeUrl }) {
  ...
  const html = shell(
    ...
    ${button('Ver todo lo de mi sector', `${SITE_URL}/regulatorio/sector`, '#6d5aef')}
  `,
    {
      preheader:
        conPlazo > 0
