...
// =====================================================================
export function alarmasEmail({ firstName, matches = [], total = 0, tipo = 'diario', esFree = false, ajustesUrl }) {
  const MORADO = '#6d5aef';
  const n = matches.length;
  const conPlazo = matches.filter((m) => m.plazo).length;
  ...
  const filas = matches
    .map((m) => {
      ...
      const contador =
        d !== null && d >= 0
          ? `<td width="44" valign="top" style="padding-right:16px;">
               <div style="font-size:22px;font-weight:500;color:${MORADO};line-height:1;letter-spacing:-.5px;">${d}</div>
               <div style="font-size:11px;color:#b8b4ac;margin-top:2px;">${d === 1 ? 'día' : 'días'}</div>
             </td>`
          : '';
      return `
  ...
  const intro =
    tipo === 'inmediato'
      ? total === 1
        ? 'Tu alarma acaba de encontrar algo que te afecta.'
        : `Tus alarmas acaban de encontrar ${total} asuntos que te afectan.`
      : tipo === 'semanal'
        ? total === 1
  ...
  const pro = esFree
    ? `<div style="margin-top:26px;padding:14px 16px;background:#f0eefe;border-radius:10px;font-size:12.5px;color:#3c3489;line-height:1.55;">
         Con Pro te avisamos el mismo día en que se abre un plazo, no el lunes siguiente, y puedes tener hasta 3 alarmas.
  ...
  const html = shell(
    ...
    ${button('Ver mis alarmas', `${SITE_URL}/seguimiento?alarmas=1`, MORADO)}
    ${pro}
  `,
    {
      preheader: conPlazo > 0 ? `${conPlazo} con plazo abierto` : `${total} ${total === 1 ? 'novedad' : 'novedades'}`,
