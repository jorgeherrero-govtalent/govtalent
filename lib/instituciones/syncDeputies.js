...
async function getDiputadosActivosUrl() {
  ...
  // El enlace en la página es una ruta relativa (sin dominio), no absoluta.
  const match = html.match(/\/webpublica\/opendata\/diputados\/DiputadosActivos__\d+\.json/);
  if (!match) throw new Error('No se encontró el enlace del JSON de "Diputados en Activo" en la página oficial');
  return `https://www.congreso.es${match[0]}`;
}
...
export async function syncDeputiesFase1() {
  ...
  // 5. Preparar todos los diputados y guardarlos en un único envío — mucho
  // más rápido y fiable que 350 peticiones sueltas, una por una. No se toca
  // cod_parlamentario, photo_url ni email — esos los rellena la Fase 2.
  const payloads = [];
  ...
  const { error: upsertError } = await supabase.from('deputies').upsert(payloads, { onConflict: 'slug' });
  if (upsertError) throw new Error(`No se pudo guardar el lote de diputados: ${upsertError.message}`);
