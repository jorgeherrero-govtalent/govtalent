...
async function getDiputadosActivosUrl() {
  ...
  if (!match) {
    // Busca cualquier mención de "DiputadosActivos" o "opendata" en bruto,
    // para ver si el enlace existe con otro formato en vez de asumir que no está.
    const idx = html.indexOf('DiputadosActivos');
    const around =
      idx >= 0
        ? html.slice(Math.max(0, idx - 200), idx + 300).replace(/\s+/g, ' ').trim()
        : '(no aparece "DiputadosActivos" en ningún sitio del HTML)';
    const opendataIdx = html.indexOf('opendata');
    const aroundOpendata =
      opendataIdx >= 0
        ? html.slice(Math.max(0, opendataIdx - 100), opendataIdx + 300).replace(/\s+/g, ' ').trim()
        : '(tampoco aparece la palabra "opendata")';
    throw new Error(
      `No se encontró el patrón exacto. Alrededor de "DiputadosActivos": ${around} | Alrededor de "opendata": ${aroundOpendata}`
    );
  }
  return match[0];
}
