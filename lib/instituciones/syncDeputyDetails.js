...
export async function diagnoseAgendaComunicacion() {
  ...
  return {
    ...
    nivel3_select_options: nivel3Options.map((m) => ({ value: m[1], texto: m[2].trim() })),
    around_select_element_by_id: aroundSelectId,
    test_get_nivel3_status: testRes.status,
    test_get_nivel3_html_length: testHtml.length,
    test_contains_asuntos_exteriores: testHasExteriores,
    test_still_shows_presidencia: testHasPresidencia,
    test_around_result_title: (() => {
      const idx = testHtml.indexOf('agendacomTituloResultado');
      return idx >= 0 ? testHtml.slice(idx, idx + 400).replace(/\s+/g, ' ').trim() : null;
    })(),
  };
}
