...
export async function diagnoseAgendaComunicacion() {
  ...
  const cookieArray = typeof sectionRes.headers.getSetCookie === 'function' ? sectionRes.headers.getSetCookie() : [];
  const cookieHeader = cookieArray.map((c) => c.split(';')[0]).join('; ');
  ...
  const viewState = extractHiddenValue('__VIEWSTATE');
  const viewStateGenerator = extractHiddenValue('__VIEWSTATEGENERATOR');
  const eventValidation = extractHiddenValue('__EVENTVALIDATION');
  const formActionMatch = sectionHtml.match(/<form[^>]*action="([^"]*)"/);

  // En vez de adivinar qué campos ocultos hacen falta, capturamos TODOS los
  // <input type="hidden"> del formulario, tal como haría un navegador real
  // al enviarlo — puede haber más de los tres habituales.
  const allHiddenInputs = [...sectionHtml.matchAll(/<input type="hidden" name="([^"]+)"(?: id="[^"]*")? value="([^"]*)"/g)];

  // Antes de asumir los nombres, comprobamos qué hay de verdad alrededor del
  // select y del botón — la primera vez cogimos por error el selector de
  // idioma (es el primer <select> de la página), así que ahora buscamos por
  // el id correcto.
  const idxSelectTag = sectionHtml.lastIndexOf('<select', sectionHtml.indexOf('selectNivel3'));
  const selectTagRaw = idxSelectTag >= 0 ? sectionHtml.slice(idxSelectTag, idxSelectTag + 400).replace(/\s+/g, ' ').trim() : null;
  const idxBtnTag = sectionHtml.indexOf('btnN3');
  const btnTagRaw = idxBtnTag >= 0 ? sectionHtml.slice(Math.max(0, idxBtnTag - 300), idxBtnTag + 100).replace(/\s+/g, ' ').trim() : null;
  const usesDoPostBack = sectionHtml.includes('__doPostBack');
  const selectNameMatch = selectTagRaw ? selectTagRaw.match(/name="([^"]+)"/) : null;

  const selectFieldName = selectNameMatch ? selectNameMatch[1] : 'ctl00$PlaceHolderMain$DisplayMode$agendaComunicacion$selectNivel3';
  const submitFieldName = 'ctl00$PlaceHolderMain$DisplayMode$agendaComunicacion$btnN3';

  const postFields = {};
  for (const [, name, value] of allHiddenInputs) {
    postFields[name] = value;
  }
  postFields.__EVENTTARGET = '';
  postFields.__EVENTARGUMENT = '';
  postFields[selectFieldName] = '531-G'; // Ministerio de Asuntos Exteriores, como prueba
  postFields[submitFieldName] = 'Ir';

  const postBody = new URLSearchParams(postFields);

  const postUrl = 'https://www.lamoncloa.gob.es/serviciosdeprensa/agendacom/paginas/index.aspx?Nivel1=1&Nivel2=5';
  ...
  const titleMatch = postHtml.match(/agendacomTituloResultado">([^<]+)</);

  return {
    cookies_found: cookieArray.length,
    section_http_status: sectionRes.status,
    viewstate_found: !!viewState,
    eventvalidation_found: !!eventValidation,
    form_action: formActionMatch ? formActionMatch[1] : null,
    all_hidden_field_names: allHiddenInputs.map((m) => m[1]),
    select_field_name_used: selectFieldName,
    uses_do_post_back: usesDoPostBack,
    post_status: postRes.status,
    post_html_length: postHtml.length,
    post_result_title: titleMatch ? titleMatch[1] : null,
    post_contains_asuntos_exteriores_person: postHtml.includes('Albares'),
  };
}
