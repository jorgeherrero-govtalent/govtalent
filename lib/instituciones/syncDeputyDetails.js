...
export async function diagnoseAgendaComunicacion() {
  ...
  const baseUrl = 'https://www.lamoncloa.gob.es/serviciosdeprensa/agendacom/paginas/index.aspx?Nivel1=1';
  const baseRes = await fetch(baseUrl, { headers: BROWSER_HEADERS });
  ...
  // Buscamos cómo aparece un enlace con Nivel2= en el listado base, para
  // saber el patrón exacto de esos hrefs.
  const nivel2Matches = [...baseHtml.matchAll(/Nivel2=(\d+)[^"']*["'][^>]*>([^<]+)</g)].slice(0, 30);

  return {
    section_http_status: sectionRes.status,
    section_html_length: sectionHtml.length,
    section_around_del_canto: contextAround(sectionHtml, 'Del Canto') || contextAround(sectionHtml, 'Secretaria de Estado'),
    base_http_status: baseRes.status,
    base_html_length: baseHtml.length,
    base_nivel2_links_sample: nivel2Matches.map((m) => ({ nivel2: m[1], texto: m[2].trim() })),
    base_around_first_nivel2: contextAround(baseHtml, 'Nivel2='),
  };
}
