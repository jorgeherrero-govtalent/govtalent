...
export async function diagnoseAgendaComunicacion() {
  ...
  // El name= sigue la convención estándar de ASP.NET (mismo patrón que ya
  // vimos en el botón "Ir": ctl00$PlaceHolderMain$...) — no hace falta
  // extraerlo, se puede construir directamente a partir del id conocido.
  ...
  return {
    ...
    post_html_first_1000: postHtml.slice(0, 1000).replace(/\s+/g, ' ').trim(),
    post_around_error: (() => {
      const idx = postHtml.search(/error|Error/);
      return idx >= 0 ? postHtml.slice(Math.max(0, idx - 100), idx + 500).replace(/\s+/g, ' ').trim() : null;
    })(),
  };
}
