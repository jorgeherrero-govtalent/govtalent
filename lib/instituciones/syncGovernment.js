...
async function fetchComposicion() {
  ...
  if (people.length === 0) {
    // Diagnóstico: buscamos "Biografía de" tal cual en el HTML crudo, para
    // ver cómo está construido de verdad en vez de seguir adivinando.
    const idx = html.indexOf('Biografía de');
    const around = idx >= 0 ? html.slice(Math.max(0, idx - 150), idx + 600).replace(/\s+/g, ' ').trim() : '(no aparece "Biografía de" en el HTML)';
    throw new Error(`0 personas encontradas. Alrededor de "Biografía de": ${around}`);
  }
