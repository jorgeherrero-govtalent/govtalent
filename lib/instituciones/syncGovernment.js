...
function isNoise(line) {
  ...
  // (el bloque de "Compartir en" son 6 seguidas). Al quitar las etiquetas se
  // queda el marcador huérfano.
  if (/^[•\-–—*]?\s*$/.test(t)) return true;
  return NOISE.some((re) => re.test(t));
}

// Algunas fichas (Robles, por ejemplo) llevan un carrusel de fotos antes de
// la biografía. Sus pies de imagen conservan retornos de carro del HTML
// original, que el texto real de la biografía nunca tiene: es el marcador más
// fiable para distinguirlos sin listar frases una a una.
function isCaption(line) {
  return /\r/.test(line);
}

// Todas las biografías de La Moncloa arrancan con "Nacido/Nacida en...".
// Si aparece, se descarta cuanto venga antes: es cabecera o galería.
function trimPreamble(lines) {
  const idx = lines.findIndex((l) => /^nacid[oa]\s+en\b/i.test(l.trim()));
