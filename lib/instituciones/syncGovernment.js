...
// La página no tiene una clase CSS fija para el bloque de contenido, así
// que cortamos entre el primer <h2> (normalmente "Formación académica") y
// el pie de página, que sí aparece igual en todas las páginas del sitio.
async function fetchBioText(bioUrl) {
  try {
    ...
    // El contenido real de la biografía termina justo antes del bloque
    // "Más información" (enlace al ministerio) + un trozo de JavaScript del
    // botón de imprimir — cortamos ahí en vez de en el pie de página, que
    // se queda demasiado lejos y arrastra ese ruido.
    const endMarkerMatch = html.slice(startIdx).match(/M(?:&#225;|á)s\s+informaci/i);
    const endIdx = endMarkerMatch ? startIdx + endMarkerMatch.index : html.indexOf('Mapa del sitio');
