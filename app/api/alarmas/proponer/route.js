...
/**
 * Si el usuario tiene las funciones Pro.
 *
 * Las tiene quien tiene Pro propio activo O es miembro de una
 * organización Teams activa: Teams incluye la licencia Pro para cada
 * miembro. Antes solo se miraba el plan del usuario, así que un miembro
 * de Teams sin Pro propio veía candados en lo que ya estaba pagando su
 * organización.
 *
 * La regla vive en SQL (public.nivel_avisos, sql/59). Si la función
 * todavía no existe —código desplegado antes que la migración—, se cae
 * a mirar el plan del usuario, que es lo que se hacía hasta ahora.
 *
...
export default function usePlanPro() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: nivel, error } = await supabase.rpc('nivel_avisos');
      if (!error && (nivel === 'pro' || nivel === 'free')) {
        if (!cancelado) setEsPro(nivel === 'pro');
        return;
      }
